...
## 6.3 TTC Threshold Sensitivity

```{r ttc-threshold-sensitivity}
...
## 6.4 TTC Threshold Visualization

```{r ttc-threshold-plots, fig.width=12, fig.height=10}
# Performance metrics across thresholds
perf_long <- threshold_comparison %>%
  select(TTC_Threshold, Accuracy, Precision, Recall, AUC) %>%
  tidyr::pivot_longer(cols = c(Accuracy, Precision, Recall, AUC),
                      names_to = "Metric", values_to = "Value")

p1 <- ggplot(perf_long, aes(x = TTC_Threshold, y = Value, color = Metric, group = Metric)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  ...
  scale_color_brewer(palette = "Set1") +
  labs(
    title = "Performance Metrics Across TTC Thresholds",
    x = "TTC Threshold (seconds)",
    y = "Score"
  ) +
  theme_minimal() +
  theme(
...
balance_long <- threshold_comparison %>%
  select(TTC_Threshold, N_Positive_Test, N_Positive_Train) %>%
  tidyr::pivot_longer(cols = c(N_Positive_Test, N_Positive_Train),
                      names_to = "Dataset", values_to = "Count") %>%
  mutate(Dataset = if_else(Dataset == "N_Positive_Train", "Training", "Testing"))

p2 <- ggplot(balance_long, aes(x = factor(TTC_Threshold), y = Count, fill = Dataset)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  geom_text(aes(label = Count), position = position_dodge(width = 0.6), vjust = -0.5, size = 4) +
  scale_fill_manual(values = c("Training" = "#1976D2", "Testing" = "#388E3C")) +
  labs(
    title = "High-Risk Cases by TTC Threshold",
    x = "TTC Threshold (seconds)",
...
# Error distribution
error_long <- threshold_comparison %>%
  select(TTC_Threshold, FPR, FNR) %>%
  tidyr::pivot_longer(cols = c(FPR, FNR), names_to = "Error_Type", values_to = "Rate")

p3 <- ggplot(error_long, aes(x = factor(TTC_Threshold), y = Rate, fill = Error_Type)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  geom_text(aes(label = sprintf("%.4f", Rate)),
  ...
  labs(
    title = "Error Rates by TTC Threshold",
    x = "TTC Threshold (seconds)",
    y = "Error Rate",
    fill = ""
  ) +
